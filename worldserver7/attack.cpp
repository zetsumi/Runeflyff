...
//		hit=70+s_dex/10;
		...
		if((target->type==ttmob)&&(type==ttplayer))
			f=((dex * 1.6f) / (dex + monsterlist[target->getmodelid()].flee)) * (level * 1.2f / (level + target->level)) * 150.0f;
		else if((target->type==ttplayer)&&(type==ttmob))
			f=(level * 0.5) / (target->level * 0.3f + level) * ((hit * 1.5f) / (target->dex * 0.5f + hit)) * 200.0f;
		else if((target->type==ttplayer)&&(type==ttplayer))
			f=((dex * 1.6f) / (dex + (10) * (level * 1.2f / (level + target->level)))) * 120.0f;
		else f= rnd(100);
