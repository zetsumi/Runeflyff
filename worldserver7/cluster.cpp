...
void cluster::cmain()
{
	...
	int a,b;
	bool schsh=false;
	long long lastclupdate=0, schs=0;
		...
		{
			{
...

				{
					...
					{
						p=loaded.pop();
//						logger.log("Player out of loaded list\n");
						dsplayers.push_front(p);
						p->dslistpos=dsplayers.begin();
						toprocess.push(p);
						startrecv.push(p);
//						logger.log("player added to ingame list\n");
					}
