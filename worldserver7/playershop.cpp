...
//		owner->bs->cmd(owner->id, 0x43) << (char)1;
		int a;
		for(a=0;a<(int)itemek.size();a++)
		{
			itemek.at(a)=255;
			nitemek.at(a)=0;
			price.at(a)=0;
            ...
            *p->bs << (char)c;
            if(c>0)
			{
                for(b=0;b<(int)itemek.size();b++)
				{
					...
					{
						*p->bs << (char)b << (unsigned int)itemek.at(b);
						...
						targy.snddata(p->bs);
                        *p->bs << (unsigned int)nitemek.at(b) << (int)price.at(b);
                    }
                }
            }
            *p->bs << 1;
			p->greentext("shopteszt\n");
	}
...
void playershop::logout(tplayer *p)
{
	...
	nplayers--;
	playershop::players.erase(p->getId());
	p->joinedshop=0;
...
void playershop::additemtoshop(int a, int b, int c, int d)
{
	...
	if(!opened)
	{
		if(owner->inv.getItem(b)==0)return;
