...
int reciever::recieve()
{
	...
	{
#ifdef use_s
		a=s_recv::recv(sck, (char*)&header[rhead], 9-rhead, 0);
#else
...
//				throw std::range_error(std::string("Reciever invalid packet size: ")+toString(torecieve));
				logger.log("Reciever invalid packet size %d", torecieve);
				return 2;
			}
	...
	{
#ifdef use_s
		a=s_recv::recv(sck, (char*)&puffer[rpuffer], torecieve-rpuffer, 0);
#else
