...
bool tparty::create1(tplayer *p, tplayer *leader, int t)	//invited player calls this
{
...

	{
		...
		memberdata.resize(g_maxpartymembers);
		members.resize(g_maxpartymembers);
		for(int a=0;a<g_maxpartymembers;a++)members.at(a)=&memberdata.at(a);
		maxpartymembers=8;
		used=true;
		ticket=t;
	}
...
void tparty::memberassign(int id, int nid, tplayer *p)
{
	...
	if((members.at(a)->pl!=0)&&(members.at(a)->dbid==id))
	{
		...
		if((members[b]->pl!=0)&&(members[b]->dbid==nid))
		{
			if(alink!=0)
			{
				members[0]->pl->cl->removelink(alinkr);
				alink=0;
